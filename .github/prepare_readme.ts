    ...
    return new Promise((resolve: (value: void) => void) => {
        ...
        reader.addListener("line", (line: string) => {
            if(lineCount++ >= 2) {
                ...
                line.match(/(?<=!\[.+\]\().+(?=\))/g)?.forEach((fileName: string) => replacedLine = replacedLine.replace(fileName, `https://raw.githubusercontent.com/Gakuto1112/TabletClockServer/main/.github/README_images/${fileName.substring(16, fileName.length)}`));
