...
## npmコマンド
このパッケージにはいくつかのnpmコマンドが定義されています。npmコマンドはこのレポジトリを直接クローンした場合のみ利用可能です（npmからインストールした場合はnpmコマンドを利用する必要がありません）。プロジェクトのルートディレクトリでnpmコマンドを実行して下さい。npmコマンドを始めて実行する場合は、実行前に以下のコマンドを実行して下さい。
